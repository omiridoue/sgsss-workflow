...
rural_schools <- school_info[school_info[,c("rural")]==1,c("school")]
non_rural_schools <- school_info[school_info[,c("rural")]==0,c("school")]

siena_effects <- output 

indices <- str_extract(names(siena_effects), "school[0-9]+")
siena_effects <- siena_effects[indices %in% non_rural_schools]
...
df <- as.data.frame(matrix(par_standers, nrow=21, ncol=length(siena_effects)))
colnames(df) <- colnames(parameters)
output_fishers1 <- c()
output_fishers1 <- cbind(df, round(Fishers, 3))

saveRDS(output_fishers1, file=paste0("${projectDir}","/results/siena_results/","${period}","/siena_fishers_subgroup_non_rural.RDS"))

siena_effects <- output
indices <- str_extract(names(siena_effects), "school[0-9]+")
siena_effects <- siena_effects[indices %in% rural_schools]
...
if (length(siena_effects)>0) { 
    ...
    colnames(df) <- colnames(parameters)
    output_fishers2 <- c()
    output_fishers2 <- cbind(df, round(Fishers, 3))

    saveRDS(output_fishers2, file=paste0("${projectDir}","/results/siena_results/","${period}","/siena_fishers_subgroup_rural.RDS"))
}
