...
influenceTable.basis <- function(x, xd, netname, behname,
  ...
  {
    require(RSiena)
    theta <- RSiena:::sienaFitThetaTable(x, fromBayes=TRUE,
...
influenceMatrix <- function(x, xd, netname, behname,
  ...
  if (is.null(levls))
  {
    levls <- unique(stab$zego)
  }
  if (is.null(levls.alt))
  {
    levls.alt <- unique(stab$alter)
  }
...
influenceTable.plot <- function(x, xd, netname, behname,
                                levls=NULL, levls.alt=levls, separation=0, bw=FALSE,
                                labs=levls.alt, beh.label=behname,
                                ylabel="evaluation function", title=NULL, ...){
  ...
  # if they overlap too much without it. An advisable value then is 0.01.
  # beh.label is the name plotted for the behaviour;
  # ylabel is the text plotted at the vertical axis;
  # title is plot title, default "Influence effect netname on behname"
  # (where netname and behname are filled in).
  # labs are labels for the curves corresponding to alter's average or constant values.
  require(ggplot2)
  zselect <- influenceTable(x, xd, netname, behname, levls, levls.alt, ...)
  quad <- attr(zselect, "quad")
...
# png(filename=paste("influenceTable_",behname,".png",sep=""), width=800,height=600)
# influenceTable.plot(x, xd, netname, behname, separation=0)
# If the curves overlap, try separation=0.01
# graphics.off()
