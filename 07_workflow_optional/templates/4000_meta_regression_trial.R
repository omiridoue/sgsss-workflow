...
control_schools <- school_info[school_info[,c("groupitt")]==0,c("school")]
trial_infos <- school_info[school_info[,c("groupitt")]==1,c("school")]

siena_effects <- output 

indices <- str_extract(names(siena_effects), "school[0-9]+")
siena_effects <- siena_effects[indices %in% trial_infos]
...
saveRDS(output_fishers1, file=paste0("${projectDir}","/results/siena_results/","${period}","/siena_fishers_subgroup_assist.RDS"))

siena_effects <- output
indices <- str_extract(names(siena_effects), "school[0-9]+")
siena_effects <- siena_effects[indices %in% control_schools]
...
if (length(siena_effects)>0) { 
    ...
    saveRDS(output_fishers2, file=paste0("${projectDir}","/results/siena_results/","${period}","/siena_fishers_subgroup_control.RDS"))
