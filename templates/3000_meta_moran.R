...
sim_paths <- c("${A}", "${S}", "${I}", "${N}")
sim_files <- list()
# Reorder sim_files based on the extracted directory names]
sim_files[1] <- sim_paths[str_detect(sim_paths, "_A_")]
sim_files[2] <- sim_paths[str_detect(sim_paths, "_S_")]
sim_files[3] <- sim_paths[str_detect(sim_paths, "_I_")]
sim_files[4] <- sim_paths[str_detect(sim_paths, "_N_")]

# Load the data
Trial_raw <- readRDS(sim_files[[1]])
Trial_exclPI_raw <- readRDS(sim_files[[2]])
Trial_exclPS_raw <- readRDS(sim_files[[3]])
...
output <- moran_decomposition(Trial_raw, Trial_exclPI_raw, Trial_exclPS_raw, Trial_exclBoth_raw, nodes, school_period)
...
x <- c("school", "school_period", "Mean_Moran_Trial", "Mean_Moran_Trial_exclPI", "Mean_Moran_Trial_exclPS", "Mean_Moran_Trial_exclBoth", "SD_Moran_Trial", "SD_Moran_Trial_exclPI", "SD_Moran_Trial_exclPS", "SD_Moran_Trial_exclBoth", "Trial_PS_M", "Trial_PI_M", "Trial_Undetermined_M", "Trial_Control_M", "Mean_Geary_Trial", "Mean_Geary_Trial_exclPI", "Mean_Geary_Trial_exclPS", "Mean_Geary_Trial_exclBoth", "SD_Geary_Trial", "SD_Trial_exclPI", "SD_Trial_exclPS", "SD_Trial_exclBoth", "Trial_PS_G", "Trial_PI_G", "Trial_Undetermined_G", "Trial_Control_G")
colnames(temp) <- x
        
saveRDS(temp, file = paste0("${projectDir}","/results/tmp/","${school_period}", "_moran", ".RDS"))