...
A process that takes the channel as input. Independent instances (tasks) of that process are run in parallel. Each task generates an output, which is passed to another channel and used as input for the next process. We'll figure out how this works in practice with some of the practice examples in the section.
...
Let's edit the `02_hello_nextflow.nf` script and add specify the output directory, where we want Nextflow to store data files that were the result of decompressing the `tar` archive folder.
...
Add the output directory `"$projectDir"` in the process `GENERATE_READS`.
