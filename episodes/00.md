...
FAIR stands for Findable, Accessible, Interoperable, and Reusable and comprises a set of principles designed to increase the visibility and usefulness of your research to others. FAIR principles have been applied for software, here is an outline of what they stand for:
...
In practice achieving these goals is not an outcome, but a process that will bring you
...
Before we dive into the details of what they mean, let's look at some examples of bad and good file names, please note there is nuansce.

| 🔨 Let's fix this         | ✔️ Good |
| -----------------|-------------------------|
...
We can come back to this to see how we can combine handy process labels in our workflow with base functions like `paste0()` in R and concatenate literal words or values with ones that depend on the input in a given point in time.


Here are some tips for naming files within a research project, which are both human- and machine-readable `Cowles2019Filenaming,Hodge2015Filenaming`:


The short answer is ‘yes’. The project design will allow you to manage your work well for yourself (see the section: Getting Started Checklist).
...
![Template repository folder for workflow](episodes/fig/repo-boilerplate.png)
...
└── 📁workflow                <- Main workflow directory
  └── 📁apptainer          <- Container definitions for Apptainer (formerly Singularity)
    └── .gitkeep
  └── 📁bin                 <- Executable scripts used in the workflow
    └── EgoAlterTable.R
    └── InfluenceTables.R
    └── moran_decomposition.R
    └── siena07RunSimOnly.R
    └── siena07RunToConvergence.R
  ...
  └── 📁data               <- Input data files for the workflow
    ...
    └── composition_multi_period.tar.gz
    └── each_period.tar.gz
    └── multi_period.tar.gz
  └── 📁docker             <- Docker container definition and dependencies
    ...
    └── renv.lock
      ...
      └── settings.json
    └── Rscript.R
    └── rsiena_1.4.19.tar.gz
  └── 📁docs               <- Documentation, reports, and visualizations
    └── gitpod101.png
    └── report.html
    └── siena_meta_network.png
    └── timeline.html
  └── 📁modules            <- Nextflow modules for different analysis steps
    └── 📁estimation     <- Module for model estimation
      └── main.nf
    └── 📁generate_reads <- Module for generating reads
      └── main.nf
    └── 📁join_files     <- Module for joining files
      └── main.nf
    └── 📁meta_fishers   <- Module for Fisher's meta-analysis
      └── main.nf
    └── 📁meta_moran     <- Module for Moran's meta-analysis
      └── main.nf
    └── 📁meta_reg       <- Module for meta-regression
      └── main.nf
  ...
  └── 📁templates          <- Template scripts used in the workflow
    └── 1000_model_estimation_all.R
    └── 1000_model_estimation_influence.R
    └── 1000_model_estimation_no.R
    └── 1000_model_estimation_selection.R
    └── 3000_meta_analysis.R
    └── 3000_meta_moran.R
    └── 3000_moran_plots.R
    └── 4000_meta_regression_rural.R
    └── 4000_meta_regression_trial.R
    └── 4000_meta_regression.R
    └── 6000_random_effects.R
    └── README.md
