...
2. Identify an appropriate use for a Value channel and define a new channel using `ch_vl`.
3. Identify an appropriate use for a Queue channel and define a new channel using `ch_qu`.
4. Based on the definition of the channel, add your channel to the existing workflow definition.
5. Use `.view()` operator on the channel objects to view the contents of the channels.
6. Run the code using the following command on the terminal:
