...
Navigate to the nextflow file by changing directory to training/sgsss-workflow/scripts/. Again this will likely look different if you
have set up on your local computer. 
...
2. The pre-populated code involves x1 queue channel. Identify the name of this channel. Write comments within your script to explain what this queue channel involves. 
3. The pre-populated code also involves x4 Value channels. Identify the names of these channels, and write up comments to explain what each value channel involves.
4. Based on the definition of the channel, add your channel to the existing workflow definition.
...
In the above example, the channel has two elements. If you used the Channel.of(aligner\_list) it would have contained only 1 element `[salmon, kallisto]` and any operator or process using the channel would run once.
