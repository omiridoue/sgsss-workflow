...
We are now going to look at a sample Nextflow script that counts the number of lines in a file. Create the file `02_hello_nextflow.nf` in the current directory.

:::::::::::::::::::::::::::::::::::::::  challenge

The nextflow scripts for each lesson episode are available in the `scripts` directory created during the course setup. You should enter this folder by chanding directory. Your 
terminal will display your relative folder path, check your terminal 

```bash 
training/sgsss-workflow -> 
```

Hint: Use the `cd` command to change directory followed by the folder name or relative path you want to navigate to, ex. `cd sgsss-workflow`. If you want to go back to the top level of the training directory you can type `cd ..`. 
...
Let's make a copy of the `02_hello_nextflow.nf` script as `read_data_params.nf` and add specify the output directory, where we want Nextflow to store data files that were the result of decompressing the `tar` archive folder.
